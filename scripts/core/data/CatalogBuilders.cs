...
public static class CatalogBuilders
{
    ...
    public const int GearMinEnergyCost = -1;
    public const int GearMaxEnergyCost = 4;

    public static Dictionary BuildGear()
    {
        var table = new Dictionary();
        var seen = new HashSet<string>();
        foreach (var row in CatalogLoader.LoadArray(DataPaths.Gear))
        {
            ...
            table[id] = new Dictionary
            {
                ...
                { "bio_key", CatalogLoader.GetString(row, "bio_key") },
                { "icon", CatalogLoader.GetString(row, "icon") },
                { "image_path", AssetPaths.GearIcon(id) }
            };
        }
        GD.Print($"[Catalog] Loaded {table.Count} gear.");
        return table;
    }

    private static Array<Dictionary> ParseGearStatList(Dictionary row, string key, string id)
    {
