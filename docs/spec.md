...
3. **外掛式細胞器（Modular Organelles）**：
   - 超出本體網格拓撲範圍的特殊攻擊，做成獨立子節點掛載（PackedScene）：
     - **吞噬偽足鏈式打擊（`PseudopodChainVisual`）**：吞噬偽足發射的暫態鏈式偽足，鏈射→杯口閉合→拖回三段命中；舊偽足捕捉爪 IK 器官已併入此機制並刪除。
...
│ [槽1] [槽2]                                            │
│ [槽3] [槽4]   已用/上限＝Σ正成本／(6＋發電數)；發電件≤2    │
└────────────────────────────────────────────────────────┘
```

胞器腔室為獨立第三系統：基礎能量 6，`energy_cost ∈ [-1, 4]`（`-1`＝`+1` 發電且必帶重度負面），已用＝Σ正成本，上限＝6＋Σ發電量，合法⇔已用≤上限且件數≤4且發電件≤2。所有效果 100% 走通用 Stat（詳見 `docs/skill.md` §4、`docs/stat.md` 能量註記）。
