...
```
         ...
         ┌───────────────────────┼───────────────────────┐
         ▼                       ▼                       ▼
【通用戰鬥屬性 (Combat - 11項)】     【通用生存屬性 (Defense - 7項)】   【通用機制屬性 (Utility - 1項)】
...
所有屬性鍵名統一使用蛇形命名法（Snake_case），並在 `scripts/core/CellStats.cs` 中註冊（共 19 項）：
