...
| :--- | :--- | :--- | :--- | :--- |
| **普通 (Normal)** | 白色正圓形小囊泡 | 提供單一基礎通用屬性（如 `might +5%` 或 `max_health +10`）。 | 無代價。 | **正式實裝** |
| **魔法 (Magic)** | 藍色正圓形囊泡（較大） | 提供較高額屬性加成，或兩個相輔相成的通用屬性（如 `area +8%` ＋ `armor +2`）。 | 無代價。 | **正式實裝** |
| **稀有 (Rare)** | 金色菱形生化複合體圖騰 | 提供大幅度單項或雙項純屬性加成（如 `might +15%`、`move_speed +10%`）。 | 無代價或輕微固定數值權衡。 | **正式實裝** |
| **獨特/傳奇 (Unique)** | 橙色六邊形圖騰 | 顛覆性機制質變節點（Keystone），改變遊戲底層運算邏輯。 | 顯著機制代價。 | **[TODO in the future] 暫不實裝**<br>(避免早期破壞數值平衡，留待未來大型更新) |
