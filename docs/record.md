...
- **歷史病歷上限**：本地最多持久化保存最新 **50 筆病歷**（`MaxRecords = 50`）。超過上限時按 FIFO 自動移除最舊的**未鎖定**記錄；已歸檔鎖定的病歷免疫自動清理（全鎖定時的溢出予以保留）。
- **歸檔鎖定（Archive Lock）**：每條病歷行尾附 `🔒` 歸檔按鈕（未鎖定呈暗灰，鎖定呈金色），點擊即釘選／解釘並持久化；被鎖定的神仙局永久保存。手動清除功能已移除，存檔只增不減、只靠 FIFO＋鎖定管理。
...
- **歸檔鎖定**：行尾 `🔒` 一鍵釘選重大病歷，免疫 FIFO 自動清理，高光時刻永久保存。
