...
- [ ] **[P0] 5 大終極表觀遺傳超武 (Epigenetic Evolutions / 1:1 二合一滿級融合)**：
  - [ ] **【顆粒酶死刑】(Granzyme Apoptosis)**（穿孔素長矛 Lv.5 ＋ 溶酶體酵素）：
    - 貫穿命中敵人植入凋亡印記，1 秒後自毀引爆，向 6 個正交方向飛濺連鎖穿刺射線。
  - [ ] **【膜攻擊終結陣列】(MAC Hyper-Array)**（補體瀑布 Lv.5 ＋ 肌動蛋白微絲）：
    - 補體地雷附著於偽足末端，走位拖曳流動化學渦流帶，接觸病原體直接裂解為 ATP 經驗滴。
  - [ ] **【中和高壓風暴】(Neutralizing Tempest)**（Y 型抗體齊射 Lv.5 ＋ 調理素親和）：
    - 發射 32 枚高頻巡航抗體，命中目標間拉出高壓生化網線，造成最大生命值百分比真傷。
  - [ ] **【過氧化利維坦】(Superoxide Leviathan)**（活性氧射流 Lv.5 ＋ 線粒體超頻）：
    - 取消前方噴射，全細胞膜包裹青藍色超氧等離子光膜，碰觸即融化一切非 Boss 病原體。
  - [ ] **【阿米巴原生巨口】(Amoebic Maelstrom)**（偽足猛擊 Lv.5 ＋ 趨化因子受體）：
    - 分裂為 4 根全向巨型阿米巴抓手，中心形成超大吸力生化風暴，將全屏雜兵與經驗強行拖入吞噬。
  - **影響檔案**：新增 `scripts/skills/evolutions/*.cs` (5 個超武類), `scripts/skills/SkillManager.cs`
- [ ] **[P0] 超武質變合成機制與升級介面共鳴反饋**：
  - 當主動技能達 Lv.5（Max）且持有對應被動特質時，擊殺精英掉落寶箱或升級時觸發質變。
  - 在升級卡牌介面 (`UpgradeModal.cs`) 中，若對應主動已滿級，對應被動卡牌外框泛出金色共鳴光環，角標顯示 `【超武催化劑】`。
  - 獲得超武後替換原主動槽位技能，並更新圖示、音效與粒子。
  - **影響檔案**：`scripts/core/UpgradeManager.cs`, `scripts/ui/UpgradeModal.cs`, `scripts/skills/SkillManager.cs`
- [ ] **[P1] 升級抽取池重構與解耦**：
  - 現有 `UpgradeManager.cs` 混雜了 16 個主動武器與 13 個被動特質，稀釋了核心超武構築。
  - 需依據解鎖狀態嚴格控制候選池，確保玩家每局能穩定朝著 5+5 終極超武體系進行流派成型。
  - **影響檔案**：`scripts/core/UpgradeManager.cs`
...
- [ ] **[P0] 15 分鐘標準波次時間軸與 3 分鐘高頻進階循環 (3-Minute Escalation Loop)**：
...
- [ ] **[P0] 同屏怪物上限與殺戮驅動即時回補 (Kill-Driven Dynamic Backfill)**：
...
- [ ] **[P0] 5 大地圖 09:00 次級領主 (Sub-Boss) 實體實作（必掉超武寶箱）**：
...
- [ ] **[P1] 歷史病歷介面功能完善**：
  ...
  - 最高分歷史病歷置頂展示。
  - 點擊歷史病歷查看完整細胞形態、器官、超武配裝組合與各項生化指標。
