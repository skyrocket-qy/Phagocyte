...
天賦星盤採用 **DBD 血網（Bloodweb）× 共軛焦螢光顯微（Confocal Fluorescence）** 的放射狀同心圓設計：中央為永久點亮的【全能造血幹細胞 (HSC)】細胞核，五大譜系門戶與四大核心節點環繞成「核內圈」，所有節點依極座標 `(ring, θ)` 動態生成，並以細胞骨架微管（螢光曲線）相連。

```
                         抗原感知中樞 (18°)
                                │
        造血核心 54° ──  核 (HSC)  ── 126° 造血核心
                                │
        極化纖毛 162°          90°          內質網工廠 306°
                                │
        造血核心 198° ── 顆粒活化 234° ── 270° 造血核心
```

- **核區（R0–R1）**：HSC 核心（先天生效、不耗點）＋五大 Legacy 門戶＋核心代謝節點。
- **胞質基質（R2–R3）**：中階 Stat 節點、Rare 質變與 Legacy 特質。
- **胞膜皮層（R4–R5）**：Notable 與 Unique 終極節點。

### 機制：自噬代謝壓力（Autophagic Tide）取代惡靈吞噬

- 每次花費點數會推高全盤代謝壓力：`沉默門檻環 = max(2, 5 − 已花點數 / 4)`。
- 未被投資（`branch investment == 0`）的譜系，其高於門檻的外環節點會「自噬退化」：無法購買、微管斷裂，外環較深者退化程度更高。
- 退化可逆：返還其他點數使壓力退潮，或花 1 點開啟該譜系門戶（R1）即可恢復整條通路。
- HSC 核心與各細胞起始節點永不受自噬影響，確保每個流派都能從中央重新分化。
...
- 背景深青黑 `#050B14`，中心以徑向漸層發出冷青螢光，搭配布朗運動塵埃。
- 連線為半透明微管曲線；已點亮路徑沿線流動 ATP 生物電脈衝；跨譜系長距離微管自動繞開細胞核。
- 節點為有機囊泡：依稀有度呈現圓形／菱形／六角／星形，並帶有微幅布朗漂移（越外環漂移越明顯）。
- 視角具彈簧延遲：拖曳星盤時節點有流體慣性回彈。
...
- [ ] 實作極座標放射狀血網佈局（同心圓 R0～R5 + 中央永久 HSC 核心）
- [ ] 實現中央幹細胞向五大起點門戶分化邏輯（核 → 各譜系 R1 門戶）
- [ ] 實作自噬代謝壓力：未投資譜系外環節點退化鎖定，返還點數或開啟門戶即可恢復
- [ ] 實現出門小節點通用 `StatModifier` 累加計算
- [ ] 實現五大特化核心關鍵節點
- [ ] 實作共軛焦螢光渲染：微管脈衝、囊泡節點、布朗漂移、彈簧慣性視角
