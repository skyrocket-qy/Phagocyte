...
天賦星盤採用 **方格電路板（Grid Board）× 共軛焦螢光顯微（Confocal Fluorescence）** 的正交網格設計：中央為永久點亮的【全能造血幹細胞 (HSC)】細胞核，每個節點佔據單一格點，連線僅存在於相鄰節點之間（垂直／水平），因此微管永不重疊、永不交叉。

```
              單核巨化區域(左上)        抗原感知中樞(上)
                     ┌───────┬──────────────┐
      顆粒活化區域   │ HSC 核心 │   極化纖毛區域 (右)
                     └───────┴──────────────┘
                            內質網工廠 (右下)
```

- **核心層（L1）**：HSC 核心（先天生效、不耗點）＋核心代謝節點，由中央向四方伸出四條主幹。
- **門戶層（L2–L3）**：五大 Legacy 譜系門戶與中階 Stat 節點。
- **外層（L4+）**：Rare／Unique／Notable 終極節點；層數 = 節點到核心的曼哈頓距離 `|col| + |row|`。

### 節點購買規則

- 每個節點僅能購買一次（單層堆疊），且一律消耗 1 點被動點數；HSC 核心為先天生效、不耗點。
- 節點效果依稀有度分層：普通 1 條小數值、魔法 1 條大數值、稀有 2 條數值、獨特為多條終極加成。
...
- [ ] 實作正交網格棋盤佈局（五大譜系橫向帶 + 中央 HSC 核心，單格單節點）
- [ ] 實現中央幹細胞向五大起點門戶分化邏輯（核 → 核心代謝環 → 各譜系門戶）
- [ ] 實現出門小節點通用 `StatModifier` 累加計算（單次購買、單一效果）
- [ ] 實現五大特化核心關鍵節點
- [ ] 實作共軛焦螢光渲染：正交微管脈衝、囊泡節點、棋盤格背景（保證無重疊、無交叉）
